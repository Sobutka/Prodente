...
export class DentistaController {
  async create(req: Request, res: Response) {
    let { nome, cro, email, dataNasc, celular, celularRecado, uf } = req.body;
    dataNasc = new Date(
      dataNasc.replace(/(\d{2})\/(\d{2})\/(\d{4})/, "$2/$1/$3"),
    );
  ...
  async listId(req: Request, res: Response) {
    try {
      ...
      const dentista = await dentistaRepository.findOneBy({
        codDent: Number(codDent),
      });
      return res.json(dentista);
    } catch (error) {
      console.log(error);
  ...
  async update(req: Request, res: Response) {
    try {
      ...
      if (dentista.affected == 1) {
        const dentistaUpdated = await dentistaRepository.findOneBy({
          codDent: Number(),
        });
  ...
  async delete(req: Request, res: Response) {
    try {
      ...
      if (dentista.affected == 1) {
        const dentistaDeleted = await dentistaRepository.findOneBy({
          codDent: Number(),
        });
