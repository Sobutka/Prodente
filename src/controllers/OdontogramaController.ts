...
export class OdontogramaController {
  async create(req: Request, res: Response) {
    ...
    try {
      const newOdontograma = odontogramaRepository.create({
        dataRealizacao,
        dente,
        face,
        valor,
  ...
  async listId(req: Request, res: Response) {
    try {
      ...
      let odontogramas = await odontogramaRepository.findOneBy({
        codOdon: Number(codOdon),
      });
      return res.json(odontogramas);
    } catch (error) {
      console.log(error);
  ...
  async update(req: Request, res: Response) {
    try {
      ...
      if (odontograma.affected == 1) {
        const odontogramaUpdated = await odontogramaRepository.findOneBy({
          codOdon: Number(),
        });
  ...
  async delete(req: Request, res: Response) {
    try {
      ...
      if (odontograma.affected == 1) {
        const odontogramaDeleted = await odontogramaRepository.findOneBy({
          codOdon: Number(),
        });
